...
class User:
    ...
    def _give_money(self, money):
        if not self._check_money_enough(money):
            raise Exception("잔돈이 부족합니다.")
        self._money -= money
...
if __name__ == "__main__":
    store_a = GrabStore(
        products={
            1: Product(name="키보드", price=30_000),
            2: Product(name="모니터", price=50_000),
        }
    )
    store_b = FruitStore(
        products={
            1: Product(name="바나나", price=3_000),
            2: Product(name="사과", price=5_000),
        }
    )
    ...
    print(f"user_a의 잔돈: {user_a.get_money()}")
    print(f"user_b의 잔돈: {user_b.get_money()}")
